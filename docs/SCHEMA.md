...
| `zenodo_doi`            | string                       | no       | DOI generated by Zenodo after publish (e.g. `10.5281/zenodo.1234`). Populated by the sync job; editor read-only. |
| `zenodo_record_id`      | string                       | no       | Published Zenodo record id. Populated by the sync job.                                                        |
| `zenodo_concept_recid`  | string                       | no       | Zenodo concept/version-family id.                                                                             |
| `zenodo_record_url`     | string                       | no       | Public Zenodo landing page / DOI URL shown when file embedding is unavailable.                                |
| `zenodo_deposition_id`  | string                       | no       | Internal deposition id used for retrying draft/published metadata sync.                                       |
| `zenodo_sync_status`    | enum                         | no       | `not_synced \| draft \| uploading \| published \| paused \| failed`. Maintained by the sync job.              |
| `zenodo_synced_at`      | datetime                     | no       | Last successful file+record publish to Zenodo.                                                               |
| `zenodo_metadata_synced_at` | datetime                 | no       | Last successful metadata-only sync from Roufouf to Zenodo.                                                   |
| `zenodo_metadata_hash`  | string                       | no       | Hash of the Roufouf fields mirrored to Zenodo; used to detect edits.                                          |
| `zenodo_sync_error`     | text                         | no       | Last sync error for admins.                                                                                   |
...
| `zenodo_file_url` | string \| null | Public Zenodo file URL for this slot. When `ops_settings.public_pdf_source = zenodo`, the website uses this for preview/download. |
| `zenodo_file_key` | string \| null | Filename/key uploaded to the Zenodo bucket. |
| `zenodo_file_checksum` | string \| null | Checksum returned by Zenodo so the archived copy can be compared with the Directus backup. |
| `label_ar` / `label_fr` / `label_en` | string | Optional display labels per locale. |

**Public resolution** first checks **`ops_settings.public_pdf_source`**. In `zenodo` mode, a slot with `zenodo_file_url` is embedded/linked from Zenodo; if only `zenodo_record_url` exists, the UI shows “View on Zenodo”. In `directus` mode, or when Zenodo fields are absent, it applies **`documents.pdf_public_display`** to the local Directus file (`auto` prefers **`optimized_file`** when **`optimization_status === 'ready'`**, else **`original`** (`file`)).
...
- **`files`** — Field Note: **`file`** = local publisher-original PDF / offline backup; **`optimized_file`** (when present) = machine derivative; **`optimization_*`** slots are maintained by **`pdf-optimize-worker`**; **`zenodo_*`** slots are maintained by the Zenodo sync job. Editors can edit JSON for labels and slot ids; uploads use standard File flows.
...
- Admin-only collection in production: notifications, backup tuning, **`public_pdf_source`** (`zenodo` or `directus`; controls the public PDF delivery source and pauses Zenodo writes when set to `directus`), **`branding_site_base_url`** (canonical public origin of the Next.js app **without** trailing slash, e.g. `https://roufouf.tn` — used by seeded **Flows** when building `POST …/api/admin/…` URLs), **`branding_webhook_secret`** (shared secret for Directus Flows calling the
